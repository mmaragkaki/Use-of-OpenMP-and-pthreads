...
int main(int argc, char *argv[])
{
  ...
  int choise;
  numofcells = 0;
  while ((choise = getopt_long_only(argc, argv, "a:b:c:d:e:f:", long_options, NULL)) != -1)
  	{
  		switch(choise)
  		{
  			case 'a':
    ...
    int array[2] = {Qmax,Dall};
    pthread_t numofthreads[INT4] ;
    for(k = 0; k < Pairs; k++){

      for(int l = 0; l < INT4; l++){
        wakeup = 0;
        pthread_create(&numofthreads[l],NULL,(void*)ParallelMain,(void*)array);
        while(!wakeup){
          ;
        }
      }

      for(int p = 0; p < INT4; p++){
      pthread_join(numofthreads[p],NULL);
    }
  }
