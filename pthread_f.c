...
char *ID,*PATH;
double elapsed_time, cellstime, cellstime1, tracetime, tracetime1;
int traceback_steps;
int **Score;
...
typedef struct def{
  char* strings_array[2];
  int j;
  int lx;
  int ly;
}def;

typedef struct defpthread2{
  char* strings_array[2];
  int lx;
  int ly;
}defpthread2;
...
int Smith_Waterman(defpthread2 pth_str);
...
double gettime(void);
int pthread2(char* X,char* Y,int lx,int ly);
void* ParallelTask(def* args);
FILE *fptr;
FILE *fid;
int wakeup;
...
int main(int argc, char *argv[])
{
  ...
  printf("Total time of execution with pthreads: %lf\n", elapsed_time );
...
int Smith_Waterman(defpthread2 pth_str){
  ...
  pthread_t numofthreads[INT4];
  def args[1];
  char* D = pth_str.strings_array[0];
  char* Q = pth_str.strings_array[1];
  int lx = pth_str.lx;
  int ly = pth_str.ly;

  int* ret_val = (int*)malloc(sizeof(int));

  int threads = 0, max_val = 0;
  int n1 = 0, n2 = 0, n3 = 0; 

  args[0].strings_array[0] = D;
  args[0].strings_array[1] = Q;
  args[0].lx = lx;
  args[0].ly = ly;
  ...
  for(int j = 0; j < threads; j++){
    args[0].j = j;
    wakeup = 0;
    pthread_create(&numofthreads[j],NULL,(void*)ParallelTask,(void*)args);
    while(!wakeup){}

  }
  for(int j = 0; j < threads; j++){
        pthread_join(numofthreads[j],&ret_val);
        max_val =  *ret_val;
    }
  free(ret_val);
  ret_val = NULL;
...
void* ParallelTask(def* args){
  int max_val = 0, n1 = 0, n2 = 0, n3 = 0, start = 0, stop = 0;
  int* tmp_max_val = (int*)malloc(sizeof(int));
  wakeup = 1;
  char *X = args[0].strings_array[0];
  char *Y = args[0].strings_array[1];
  int j = args[0].j;
  int lx = args[0].lx;
  int ly = args[0].ly;

  start = (int)((lx+INT4-1)/INT4)*j;
  stop =  (int)((lx+INT4-1)/INT4)*(j+1);
...
int pthread2(char* X,char* Y,int lx,int ly){
  defpthread2 pth_struct;

  //Filling my struct
  pth_struct.strings_array[0] = X;
  pth_struct.strings_array[1] = Y;
  pth_struct.lx = lx;
  pth_struct.ly = ly;

  int max = 0 ;
  double time0 = gettime();
  max = Smith_Waterman(pth_struct);
  double time1 = gettime();
